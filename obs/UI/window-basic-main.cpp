...
static void AddSource0(void *_data, obs_scene_t *scene)
{
...
void OBSBasic::doTest()
{
    ...
    while (obs_enum_input_types(idx++, &type))
    {
        ...
        qDebug()<<QString("name=%1,type=%2,cap=%3").arg(name).arg(type).arg(caps);
        if(caps == 137)
        {
            break;
        }
    ...
    qDebug()<<QString("placeHolderText=%1").arg(name);

    obs_source_t *source = obs_get_source_by_name(QT_TO_UTF8(name));
    if (source) {
        ...
        source = obs_source_create(id, QT_TO_UTF8(name), NULL, nullptr);
...
//                obs_sceneitem_defer_update_end(item);
            ...
            obs_scene_enum_items(scene,func,this);
            //                newSource = source;
...
void OBSBasic::on_actionCopyTransform_triggered()
{
    auto func = [](obs_scene_t *scene, obs_sceneitem_t *item, void *param)
    {
        ...
        qDebug()<<QString("on_actionCopyTransform_triggered left=%1,%2,=%3,=%4").arg(copiedCropInfo.left).arg(copiedCropInfo.right).arg(copiedCropInfo.top).arg(copiedCropInfo.bottom);
        obs_sceneitem_defer_update_end(item);
